...
###
	...
	ranged_data<-c(min(tab_sample_a_order),range_colors[,1],1.5,max(tab_sample_a_order))
	print(length(ranged_data))

	#if 100% of data are lower than 0.80
	if(ranged_data[6]<=1.5){

	color.scale<-c(c("white","white","white","white","white","white"), paste(color,3,sep="")) } else {

	color.scale<-c(c("white","white","white","white","white"), paste(color,seq(2:3),sep=""))
	...
	range_colors<-data.frame(quantile(apply(difference_tab,2,mean)))
        ranged_data<-c(min(difference_tab),range_colors[,1],1.5,max(difference_tab))

	main_diff<-paste(sample_b_label,sample_a_label,sep="/")

	color.scale<-c(c("white","white","white","white","white","white"), paste(color,3,sep=""))
...
rm(difference_tab)
rm(tab_sample_a_order)
rm(tab_sample_b_order)
rm(ranged_data)
rm(range_colors)
